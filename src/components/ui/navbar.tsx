    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg px-4 py-8 border border-white/10",
...
export const NavbarLogo = ({ name }: NavbarLogoProps) => {
  const { theme } = useTheme();
  ...
  return (
    ...
    >
      <span className={cn("font-medium", theme.text)}>{name}</span>
    </Link>
  );
};
